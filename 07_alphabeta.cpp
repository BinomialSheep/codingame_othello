...
ScoreType alphaBetaScore(State& state, ScoreType alpha, const ScoreType beta,
  ...
  if (legal_actions.empty()) {
    state.isBlack ^= 1;
    auto ret = -alphaBetaScore(state, -beta, -alpha, depth - 1);
    state.isBlack ^= 1;
    return ret;
  }
  ...
  for (const auto action : legal_actions) {
    ...
    ScoreType score = -alphaBetaScore(state, -beta, -alpha, depth - 1);
...
int alphaBetaAction(State& state, const int depth) {
...
int main() {
  ...
  while (1) {
    ...
    int res = alphabeta::alphaBetaAction(state, 2);
