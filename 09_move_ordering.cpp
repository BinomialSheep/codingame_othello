...
/*
iteradiveDeepingで140ms間alphaBeta。
Wood1昇格。
*/
...
ScoreType alphaBetaScore(State& state, ScoreType alpha, const ScoreType beta,
  ...
  if (provisionalScores[!state.isBlack][state.board.black][state.board.white]
          .empty()) {
    ...
    for (auto [score, action] : score_acion) {
      ...
      st.insert(action);
      // cerr << depth << " " << score << " " << action << endl;
    }
    vector<int> tmp = state.legalActions();
    for (auto action : tmp)
      if (!st.count(action)) legal_actions.push_back(action);
    assert(tmp.size() == legal_actions.size());
    provisionalScores[!state.isBlack][state.board.black][state.board.white]
        .clear();
  }
