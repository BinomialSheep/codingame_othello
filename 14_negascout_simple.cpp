...
/*
negascoutをバグなく入れるための試験的なバージョン（弱い）
*/
...
// 検証のための簡易実装
ScoreType negaScoutMiniState(State& state, ScoreType alpha, ScoreType beta,
  ...
  if (legal_actions.empty()) {
    if (passed) {
      // ゲーム終了
      int cntB = __builtin_popcountll(state.board.black);
      int cntW = __builtin_popcountll(state.board.white);
      if (cntB == cntW) return 0;
      if (state.isBlack && cntB > cntW) return INF;
      if (!state.isBlack && cntB < cntW) return INF;
      return -INF;
    }
    state.isBlack ^= 1;
    auto ret =
        -negaScoutMiniState(state, -beta, -alpha, true, depth - 1, time_keeper);
    state.isBlack ^= 1;
    return ret;
  }
  ...
  // 最善候補は普通にαβ
  {
    ...
    ScoreType score = -negaScoutMiniState(state, -beta, -alpha, false,
                                          depth - 1, time_keeper);
  ...
  for (int i = 1; i < (int)legal_actions.size(); i++) {
    ...
    const uint64_t flips = state.advance(action);
    // 幅1[alpha, alpha+1]で
    ScoreType score = -negaScoutMiniState(state, -alpha - 1, -alpha, false,
                                          depth - 1, time_keeper);
    state.retreat(action, flips);
    // betaすら超えていたら再探索不要
    if (score >= beta) return score;
    // 最善手候補よりも良い手が見つかった場合は通常の窓で探索
    if (chmax(alpha, score)) {
      state.advance(action);
      score = -negaScoutMiniState(state, -beta, -alpha, false, depth - 1,
                                  time_keeper);
      state.retreat(action, flips);
      // 興味の範囲よりもminimax値が上のときは枝刈り fail high
      if (score >= beta) return score;
