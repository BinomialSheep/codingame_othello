...
int main() {
  ...
  // vector<string> a = {"........", "........", "........", "..000...",
  //                     "...01...", "....01..", "........", "........"};
  // OthelloBoard board(a);
  // board.printBoard();
  // board.printBinary(board.right_mask);
  // board.printBinary(board.left_mask);
  // board.printBinary(board.diag_mask);
  // auto tmp = board.findLegalMovesIdx(board.white, board.black);
  // cerr << tmp.size() << endl;
  // return 0;
  ...
  while (1) {
    ...
    vector<int> idxes = board.findLegalMovesIdx(isBlack);
    cerr << actions.size() << " " << idxes.size() << " " << endl;
    if (actions.size() != idxes.size()) {
      for (auto v : actions) cerr << v << " ";
      cerr << endl;
      for (auto v : idxes) cerr << v << " ";
      cerr << endl;
      rep(i, 8) cerr << sboard[i] << endl;
    }
    assert(actions.size() == idxes.size());
    if (actions.size() == 0) continue;

    int maxEvalValue = -1e9, idx = -1;
    rep(i, actions.size()) {
      int pos = (actions[i][1] - '1') * 8 + (actions[i][0] - 'a');
      uint64_t position = 1ULL << pos;
      uint64_t flips = board.calculateFlips(position, isBlack);

      board.placeStone(position, flips, isBlack);
      // board.printBoard();
      int score = board.evaluate(isBlack);
      cerr << actions[i] << " " << score << endl;
      board.revPlaceStone(position, flips, isBlack);
      // board.printBoard();

      if (chmax(maxEvalValue, score)) {
        idx = i;
      }
    }
